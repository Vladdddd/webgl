...
function Model(name) {
  ...
  this.Draw = function () {
    ...
    for (let i = 0; i < lines * 2; i++) {
      gl.drawArrays(gl.LINE_STRIP, this.segments * i, this.segments);
    }
  };
}
...
function CreateShoeSurfaceData() {
  ...
  for (let i = min; i < max; i += step) {
    for (let j = min; j < max; j += step) {
      vertexList.push(i, j, i ** 3 / 3 - j ** 2 / 2);
    }
  }

  for (let i = min; i < max; i += step) {
    for (let j = min; j < max; j += step) {
      vertexList.push(j, i, j ** 3 / 3 - i ** 2 / 2);
    }
  }
...
function initGL() {
  ...
  surface.BufferData(CreateShoeSurfaceData());
